package cli

import (
	"context"
	"encoding/json"
	"errors"
	"fmt"
	"io"
	"sync"
	"time"

	"github.com/artschekoff/slack-cli/internal/credentials"
	"github.com/artschekoff/slack-cli/internal/slack"
	"golang.org/x/sync/errgroup"
)

// dmResult is one entry in the JSON output of list-dms.
type dmResult struct {
	ID          string     `json:"id"`
	UserID      string     `json:"userId,omitempty"`
	UserName    string     `json:"userName,omitempty"`
	Name        string     `json:"name,omitempty"`
	IsIM        bool       `json:"isIm"`
	LastMessage *dmMessage `json:"lastMessage,omitempty"`
}

// dmMessage is the latest message in a DM, included when --with-messages is set.
type dmMessage struct {
	UserID    string `json:"userId"`
	Text      string `json:"text"`
	Timestamp string `json:"timestamp"`
}

// ListDMsCommand lists Slack direct message conversations (1:1 and group DMs)
// and resolves user IDs to display names for 1:1 DMs.
type ListDMsCommand struct {
	Store         *credentials.Store
	Output        io.Writer
	ClientFactory ClientFactory
	WithMessages  bool
	SystemEvents  bool // when true, system/bot messages are included instead of filtered
}

// Run fetches DM conversations via conversations.list, resolves user IDs for
// 1:1 DMs concurrently, and writes a JSON array to Output.
//
// When startFrom is non-zero, messages are fetched for every DM and only those
// with a qualifying user message on or after startFrom are included. This is
// necessary because the Slack conversations.list API does not expose a reliable
// last-activity timestamp.
//
// When WithMessages is true (--with-messages), the latest qualifying message is
// included in the output for each DM.
func (c *ListDMsCommand) Run(ctx context.Context, workspace string, startFrom time.Time) error {
	client, err := resolveClient(ctx, c.Store, workspace, c.ClientFactory)
	if err != nil {
		return err
	}

	dms, err := client.ListDMs(ctx)
	if err != nil {
		if errors.Is(err, slack.ErrUnauthorized) {
			return &unauthorizedError{workspace: workspace}
		}
		return fmt.Errorf("listing DMs: %w", err)
	}

	nameMap := resolveDMUsers(ctx, client, dms)

	// Fetch messages when we need them for filtering (startFrom) or display (WithMessages).
	needMessages := c.WithMessages || !startFrom.IsZero()
	var msgMap map[string]*dmMessage
	if needMessages {
		msgMap = fetchLatestMessages(ctx, client, dms, c.SystemEvents, startFrom)
	}

	results := make([]dmResult, 0, len(dms))
	for _, dm := range dms {
		if msgMap != nil && msgMap[dm.ID] == nil {
			continue
		}
		r := dmResult{
			ID:   dm.ID,
			IsIM: dm.IsIM,
		}
		if dm.IsIM {
			r.UserID = dm.UserID
			name := nameMap[dm.UserID]
			if name == "" {
				name = dm.UserID
			}
			r.UserName = name
		} else {
			r.Name = dm.Name
		}
		if c.WithMessages && msgMap != nil {
			r.LastMessage = msgMap[dm.ID]
		}
		results = append(results, r)
	}

	return json.NewEncoder(c.Output).Encode(results)
}

const (
	maxConcurrentMessageFetches = 10
	latestMessageBatch          = 5
)

// fetchLatestMessages fetches the most recent non-system, non-bot message for
// each DM concurrently. A small batch is retrieved so that system notifications
// (channel_join, etc.) and bot messages can be skipped. When includeSystem is
// true, no filtering is applied. When startFrom is non-zero, only messages with
// a RawTS on or after startFrom are considered; DMs with no qualifying message
// are omitted from the result map.
func fetchLatestMessages(ctx context.Context, client *slack.Client, dms []slack.DM, includeSystem bool, startFrom time.Time) map[string]*dmMessage {
	result := make(map[string]*dmMessage, len(dms))
	var mu sync.Mutex

	g, gCtx := errgroup.WithContext(ctx)
	g.SetLimit(maxConcurrentMessageFetches)
	for _, dm := range dms {
		g.Go(func() error {
			limit := latestMessageBatch
			msgs, _, err := client.GetChannelMessages(gCtx, dm.ID, limit)
			if err != nil || len(msgs) == 0 {
				return nil
			}
			for _, m := range msgs {
				if !includeSystem && (m.IsSystemMessage() || m.IsBotMessage() || m.IsSlackbotMessage() || m.IsAutoGenerated() || m.IsWorkspaceJoinIntro()) {
					continue
				}
				if m.Text == "" {
					continue
				}
				if !startFrom.IsZero() && slack.TsUnixSec(m.RawTS) < startFrom.Unix() {
					continue
				}
				mu.Lock()
				result[dm.ID] = &dmMessage{
					UserID:    m.UserID,
					Text:      m.Text,
					Timestamp: m.Timestamp,
				}
				mu.Unlock()
				return nil
			}
			return nil
		})
	}
	_ = g.Wait()
	return result
}

// resolveDMUsers resolves the unique set of user IDs from 1:1 DMs to display names.
func resolveDMUsers(ctx context.Context, client *slack.Client, dms []slack.DM) map[string]string {
	seen := make(map[string]struct{})
	var uids []string
	for _, dm := range dms {
		if !dm.IsIM || dm.UserID == "" {
			continue
		}
		if _, ok := seen[dm.UserID]; !ok {
			seen[dm.UserID] = struct{}{}
			uids = append(uids, dm.UserID)
		}
	}
	if len(uids) == 0 {
		return nil
	}

	cache := slack.NewUserCache(client)
	nameMap := make(map[string]string, len(uids))
	var mu sync.Mutex

	g, gCtx := errgroup.WithContext(ctx)
	g.SetLimit(maxConcurrentUserResolves)
	for _, uid := range uids {
		g.Go(func() error {
			name, resolveErr := cache.Resolve(gCtx, uid)
			if resolveErr != nil {
				name = uid
			}
			mu.Lock()
			nameMap[uid] = name
			mu.Unlock()
			return nil
		})
	}
	_ = g.Wait()
	return nameMap
}
