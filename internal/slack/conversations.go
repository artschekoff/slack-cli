...
var systemMessageSubtypes = map[string]struct{}{
	...
	"pinned_item":       {},
	"unpinned_item":     {},
	// sh_room_created is the subtype Slack uses for the "X joined Slack — take
	// a second to say hello." message that appears in every DM when a user first
	// joins the workspace. The author field is set to the joining user's own ID
	// (not USLACKBOT and no bot_id), so it must be caught via subtype.
	"sh_room_created": {},
}
...
// slackbotUserID is the reserved Slack user ID for the built-in Slackbot.
const slackbotUserID = "USLACKBOT"

// IsSlackbotMessage reports whether the message was sent by the built-in
// Slackbot (user ID "USLACKBOT"). These messages carry no subtype and no
// bot_id, so they are not caught by IsSystemMessage or IsBotMessage.
func (m Message) IsSlackbotMessage() bool {
	return m.UserID == slackbotUserID
}

// IsAutoGenerated reports whether the message was auto-generated by Slack
// rather than typed by a human. Every user-typed message carries a
// client_msg_id UUID assigned by the Slack client; auto-generated messages
// (workspace-join intros, room-created notices, etc.) have an empty
// client_msg_id even when they carry no subtype and no bot_id.
func (m Message) IsAutoGenerated() bool {
	return m.ClientMsgID == "" && m.BotID == "" && m.Subtype == "" && m.UserID != ""
}

// workspaceJoinSuffix is the invariant tail of the workspace-join intro message
// that Slack injects into every DM when a user first joins the workspace.
const workspaceJoinSuffix = "joined Slack \u2014 take a second to say hello."

// IsWorkspaceJoinIntro reports whether the message is the auto-generated
// "X joined Slack — take a second to say hello." notification. Some variants
// of this message carry a client_msg_id and no subtype/bot_id, so they bypass
// IsSystemMessage, IsBotMessage, IsSlackbotMessage, and IsAutoGenerated.
func (m Message) IsWorkspaceJoinIntro() bool {
	return strings.HasSuffix(m.Text, workspaceJoinSuffix)
}
